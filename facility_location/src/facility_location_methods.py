...
@torch.no_grad()
def egn_pb_greedy_facility_location_direct_probs(
    points,
    graph,
    dist,
    num_clusters,
...
@torch.no_grad()
def egn_pb_greedy_facility_location_direct_probs_no_kmeans(
    points,
    graph,
    dist,
    num_clusters,
...
@torch.no_grad()
def egn_pb_greedy_facility_location_direct_probs_no_kmeans_clean(
    points,
    graph,
    dist,
    num_clusters,    
    ...
    while any(improved):
        ...
        for i_input in range(random_trials):
            ...
            for best_index in vx2result_flatten_topk[i_input]:
                ...
                if torch.abs(probs_matrix[i_input, best_v] - best_x) >= 0.01:
                    probs_matrix[i_input, best_v] = 0.001 + 0.998 * best_x
                    derandomization_seq.append((best_v, best_x))
                    improved[i_input] = True    
    return derandomization_seq

@torch.no_grad()
def egn_pb_greedy_facility_location_direct_probs_no_kmeans_clean_soft(
    points,
    graph,
    dist,
    num_clusters,
...
@torch.no_grad()
def egn_pb_sequential_facility_location_direct_probs_no_kmeans_clean(
    points,
    graph,
    dist,
    num_clusters,    
    ...
    for i_node in range(n_nodes):
        ...
        vx2result_i = vx2result[:, i_node]  # [b, 2]
        best_x = torch.argmin(vx2result_i, dim=-1)  # [b]        
        probs_matrix[:, i_node] = 0.001 + 0.998 * best_x
...
@torch.no_grad()
def sequential_given_seq_entry_and_val(
    points,
    graph,
    dist,
    num_clusters,    
...
@torch.no_grad()
def egn_pb_sequential_facility_location_direct_probs_kmeans_clean(
    points,
    graph,
    dist,
    num_clusters,    
...
@torch.no_grad()
def egn_pb_greedy_facility_location_direct_probs_kmeans_clean(
    points,
    graph,
    dist,
    num_clusters,    
...
@torch.no_grad()
def facility_location_sampling_without_replacement(
    points,    
    num_clusters,
    probs,
    n_samples=1,
    ...
    for i_sample in range(n_samples):
        # sample k=num_clusters nodes from probs without replacement
        sampled_points = torch.multinomial(probs, num_clusters, replacement=False)
        cluster_centers = points[sampled_points]
        objective = compute_objective(
            points, cluster_centers, distance_metric
        ).item()
        if objective < best_objective:
            best_objective = objective
            best_selected_indices = sampled_points
...
@torch.no_grad()
def facility_location_topk_only(
    points,    
    num_clusters,
    probs,    
    distance_metric="euclidean",    
    ...
    probs_matrix = probs.unsqueeze(0)
    k = num_clusters
    ...
    for top_k_ind in top_k_indices:
        ...
        if objective < best_objective:
            best_objective = objective
            best_selected_indices = top_k_ind
        best_objective_list.append(best_objective)
...
@torch.no_grad()
def facility_location_topk_kmeans_only(
    points,
    num_clusters,
    probs,
    distance_metric="euclidean",    
